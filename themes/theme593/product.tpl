		...
		<div id="pb-right-column" class="span4">
        	...
        	<p class="resetimg">
        		<span id="wrapResetImages" style="display: none;">
        			<span>
        				<a id="resetImages" href="{$link->getProductLink($product)}" onclick="$('span#wrapResetImages').hide('slow');return (false);"><i class="icon-picture"></i>{l s='Display all pictures'}</a>
        			</span>
        		</span>
        	</p>
        	{/if}
			...
			<ul id="usefull_link_block" class="clearfix" >
				...
				<li class="print">
					<a href="javascript:print();"><i class="icon-print"></i>{l s='Print'}</a>
				</li>
            		...
            		{if $product->show_price AND !isset($restricted_country_mode) AND !$PS_CATALOG_MODE}
						...
						<div class="row-2" {if !$product->specificPrice OR $product->specificPrice.reduction_type != 'percentage'} style="display:none;"{/if}>
		            		...
		            		<p id="reduction_amount" {if !$product->specificPrice OR $product->specificPrice.reduction_type != 'amount' && $product->specificPrice.reduction|intval ==0} style="display:none"{/if}>
		            			<span id="reduction_amount_display" class="price">{if $product->specificPrice AND $product->specificPrice.reduction_type == 'amount' && $product->specificPrice.reduction|intval !=0}-{convertPrice price=$product->specificPrice.reduction|floatval}{/if}</span>
		            		</p>
